...
"""
Publica os 8 reels + 8 cards das Semis (J101-J102) na página /admin/instagram-posts.

Fluxo:
  1. Para cada jogo (89-96):
     - cria pasta 71-72_reel_semis-<time_a>-x-<time_b>/ e move mp4 + poster
     - cria pasta 73-74_card_semis-<time_a>-x-<time_b>/ e move card.png
     - gera LEGENDA.md (multilíngue com hashtags)
     - gera ROTEIRO.md pros reels
  2. Roda build_ig_manifest.mjs + upload_ig_to_supabase.mjs

Uso: python marketing/scripts/publicar_ig_semis.py
"""
...
NEXT_REEL_START = 71  # 71-72
NEXT_CARD_START = 73  # 73-74

JOGOS_ALVO = list(range(101, 103))
...
Nas Semis da Copa 2026, {votos} das {total} IAs que palpitaram este jogo apontaram **{placar}** como placar mais provável ({pct}% do consenso).

Todos os 62 palpites (ChatGPT, Claude, Gemini, Grok, DeepSeek, Perplexity, Le Chat, Meta, Copilot, Qwen, Manus, Kimi K2 e mais 51) estão públicos no site.

Vai bater as IAs? 👉 bolao.arenadasias.com.br

#BolaoDasIAs #Copa2026 #FifaWorldCup2026 #Semis #{slugify(ta).replace('-','')} #{slugify(tb).replace('-','')} #ChatGPT #Claude #Gemini #Grok #IA #InteligenciaArtificial #ArenaDasIAs
"""
...
Nas Semis da Copa 2026, {pct}% das {total} IAs que palpitaram este jogo apontaram **{placar}**.

Card mostra o consenso e como ChatGPT, Claude, Gemini e Grok se posicionaram individualmente. Todos os 62 palpites completos em bolao.arenadasias.com.br

#BolaoDasIAs #Copa2026 #FifaWorldCup2026 #Semis #{slugify(ta).replace('-','')} #{slugify(tb).replace('-','')} #ChatGPT #Claude #Gemini #Grok #IA #ArenaDasIAs
"""
...
def roteiro_reel(jogo: dict, dados: dict) -> str:
    ...
    placar = f"{bola.get('gols_a')}×{bola.get('gols_b')}" if bola else "—"
    ta, tb = jogo["time_a"], jogo["time_b"]
    return f"""# Reel — Semis J{jogo['numero']}: {ta} × {tb}
...
def main():
    ...
    for i, numero in enumerate(JOGOS_ALVO):
        ...
        # --- REEL ---
        reel_id = f"{NEXT_REEL_START + i:02d}_reel_semis-{slug_confronto}"
        reel_dir = BRAINSTORM / reel_id
        reel_dir.mkdir(exist_ok=True)
        origem_reel = REELS_DIR / f"partida-{numero:03d}"
        # Copia mp4 renomeado
        shutil.copy(
            origem_reel / f"partida-{numero:03d}.mp4", reel_dir / f"semis-{slug_confronto}.mp4"
        )
        ...
        # --- CARD ---
        card_id = f"{NEXT_CARD_START + i:02d}_card_semis-{slug_confronto}"
