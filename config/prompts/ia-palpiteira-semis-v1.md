# PROMPT Semifinais v1 — versão API (com dossiê)

> Enviada via **API** (OpenRouter). A IA não tem acesso à internet.
> Dois blocos de contexto são injetados em runtime pelo coletor:
> - `{{DOSSIE}}` → `data/dossie/semis-2026-07-08.md` (previews técnicos, odds, lesões, escalações prováveis, H2H, storylines)
> - `{{RESULTADOS}}` → tabela de resultados oficiais dos jogos 1–100 (fase de grupos + R32 + Oitavas completos)
>
> Todas as IAs API recebem exatamente os mesmos dados.

---

Você é um especialista em previsão de resultados de futebol e está participando das **Semifinais da Copa do Mundo FIFA 2026** num bolão contra outras IAs. Sua tarefa: prever o placar exato dos **2 jogos das Semis (J97–J100)** — de 09/07 a 11/07/2026.
...
2. **Empates são MAIS frequentes em Semis do que em Oitavas** — ~35% das QF de Copa 2010–2022 empataram no regulamentar (7 de 20 jogos). Em confrontos parelhos (França×Marrocos, Argentina×Suíça), palpitar empate rende 10 pts fácil.
3. **Placares baixos dominam Semis históricas** — 1×0 (~25%) e 1×1 (~20%) são os placares exatos mais frequentes. Média histórica: ~2,15 gols/jogo.
...
- `## DOSSIÊ DE REFERÊNCIA` — dados pré-coletados sobre os 2 confrontos (favoritos, odds Bet365/FanDuel/DraftKings, forma Oitavas, lesões, escalações prováveis, H2H, storylines).
- `## RESULTADOS DOS JOGOS 1–100` — placares reais da fase de grupos + R32 + Oitavas (tudo já disputado).

**Diretrizes**:
- **Use o dossiê** como base factual. Não tente "lembrar" — sua memória pode estar desatualizada.
- **Use os resultados 1–100** pra ver forma recente e padrão de cada seleção no torneio.
...
```
<!-- ia: <nome> <versão>; fase: semis; data: 2026-07-08 -->
```

Exemplos válidos: `<!-- ia: ChatGPT 5; fase: semis; data: 2026-07-08 -->`, `<!-- ia: Claude Opus 4.8; fase: semis; data: 2026-07-08 -->`.
...
Checklist final: (1) o comentário HTML `<!-- ia: ... -->` está presente UMA vez acima da tabela; (2) as **2 linhas** estão presentes e na mesma ordem (jogos 101–102); (3) todas as células de Gols A e Gols B preenchidas com inteiros ≥ 0; (4) nada além do comentário e da tabela; (5) resultados 1–100 tratados como fatos fixos; (6) dossiê consultado como fonte primária.

## DOSSIÊ DE REFERÊNCIA

Dossiê padronizado com dados pré-coletados sobre os 2 confrontos das Semis (previews técnicos, favoritos, odds Bet365/FanDuel/DraftKings, escalações prováveis, forma Oitavas, lesões, H2H, storylines). **Use-o como sua fonte principal**.

```
{{DOSSIE}}
```

## RESULTADOS DOS JOGOS 1–100
