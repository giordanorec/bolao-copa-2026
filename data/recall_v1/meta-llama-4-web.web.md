# PROMPT v2 — versão WEB (coleta manual com pesquisa nativa, resultados inline)

> Use este prompt na interface web da IA (chat.openai.com, claude.ai, gemini.google.com, grok.com, perplexity.ai, chat.deepseek.com etc.). A IA pode pesquisar internet e usar modos avançados (Deep Research / DeepSearch / Extended Thinking).
>
...
>
> **Importante:** os resultados dos jogos 1–40 já estão inline abaixo — são fatos fixos. A IA deve usar busca para **informação nova** (estado atual das seleções, lesões, suspensões, odds), mas não para "descobrir" o que já aconteceu.
>
> Cole tudo abaixo da linha tracejada.

---

# Briefing v2 — Bolão das IAs · Copa do Mundo FIFA 2026 · Fase de Grupos Restante
...
Esta é a **segunda leva de palpites ("v2")**: a Copa já começou. As rodadas 1 e 2 da fase de grupos foram disputadas. Você tem **resultados reais em mãos** (seção abaixo) e pode pesquisar informação atualizada. Use tudo isso.

Seu trabalho: prever o placar exato dos **32 jogos restantes da fase de grupos** (jogos 41–72, de 22 a 27 de junho de 2026).

**Trate isso como um problema profissional de research + decisão sob incerteza. Não como um chute casual.**

---

## 1. Resultados fixos — jogos 1–40 já disputados

**Estes resultados são fatos imutáveis. Não pesquise para "confirmar" nem tente lembrar de placares alternativos. Use-os diretamente para entender a classificação parcial de cada grupo e a forma de cada seleção.**
...
## 2. Regras de pontuação (otimize para isto, não para "vencedores acertados")
...
- **Empate é categórico**: palpitou 1×1, deu 0×0 → 5 pontos.
- Saldo só conta se o vencedor está correto.
- Mata-mata vale 2× (fora do escopo deste prompt).

Sua função-objetivo: **maximizar valor esperado de pontos no total dos 32 jogos**, não acertar mais vencedores.

---

## 3. PROTOCOLO DE PESQUISA — faça isto antes de palpitar

### 3.1 O que buscar (use sua pesquisa nativa)

Pesquise informação **atualizada e não presente nos resultados acima**. Foco:

1. **Estado atual das seleções** — lesões confirmadas ou suspeitas entre 18 e 22/06, suspensões por acúmulo de cartões (amarelos), troca de treinador, conflitos internos relevantes.
2. **Classificação detalhada de cada grupo** — pontos, gols pró/contra, saldo acumulado após as rodadas 1–2. (Você pode derivar isso dos resultados acima, mas confirme via search se tiver dúvida.)
3. **Odds de mercado** (Pinnacle, Bet365) para os jogos 41–72 — use como probabilidade agregada, não como oráculo de placar.
4. **Cenários de classificação da 3ª rodada** — quais times precisam vencer, podem avançar empatando, ou já estão classificados/eliminados. Este é o sinal mais valioso para os jogos 53–72 (rodada final simultânea de cada grupo).
5. **Sinais fracos**: escalações mais recentes, clima previsto, distância de viagem entre cidades.

**Não pesquise** os resultados dos jogos 1–40 — eles já estão fixos acima.

### 3.2 Análise dos cenários de última rodada (crítico)

Para cada grupo, **antes de palpitar a 3ª rodada**, responda:
- Quem já está classificado (pode poupar titulares)?
- Quem está eliminado (pode reduzir intensidade)?
- Existe cenário em que dois times se classificam empatando (incentivo mútuo pro empate)?
- Algum time precisa vencer por placar específico (goleada)?

Esses fatores devem influenciar diretamente seus palpites para os jogos 53–72.

### 3.3 Ativação de modos especiais

Use **explicitamente** o modo mais profundo que sua interface oferece:
- **Pense por extenso** antes de cravar cada placar
- **Conectores e ferramentas**: se você tem acesso a Google Search, Bing, Browse, Code Interpreter — use-os ativamente
- Se conseguir rodar código Python para calcular Poisson ou simulações Monte Carlo, **faça-o**

---

## 4. ESTRATÉGIA COMPETITIVA

### 4.1 Calibração de confiança

Para cada jogo, estime mentalmente sua confiança no placar modal (1 = altíssima, 5 = baixíssima):

- **Confiança 1-2** — mismatches claros ou times em forma vs fraquíssimos. Siga o consenso técnico (2×0, 2×1, 3×0).
- **Confiança 3** — favorito moderado vs azarão competente. Placar modal (1×0, 2×1) é o caminho.
- **Confiança 4-5** — jogos genuinamente equilibrados ou de última rodada com incentivos especiais. Aqui você tem mais liberdade de diferenciar-se, desde que com motivo técnico concreto.

### 4.2 Quando arriscar

Use placares incomuns mas defensáveis em **no máximo 5-8 dos 32 jogos** (~15-20%). Critérios para ativar: (1) confiança ≥ 4; (2) há motivo técnico concreto; (3) você consegue articular em 1 frase por que as outras IAs vão errar.

### 4.3 Quando aceitar o consenso

Se mercado e literatura concordam em 2×0 e você não tem motivo concreto pra discordar, palpite 2×0. Originalidade sem fundamento perde pontos.

---

## 5. MATEMÁTICA DE PONTOS

- **Placar exato dá só 2× o vencedor simples** (10 vs 5). Não force "3×1 sangue na areia".
- **Saldo importa** (7 pra vencedor+saldo vs 5 pra só vencedor). Calibrar o saldo modal vale ouro.
- **Empate é categórico** — palpitar empate vale 5 garantidos. **0×0 e 1×1 dominam** entre os empates modernos.
- **Placares modais em Copas recentes**: 1×0 (~18%), 2×1 (~14%), 2×0 (~12%), 1×1 (~10%), 0×0 (~8%), 2×2 (~6%). Combinados ≈ 68% dos jogos.
- **Última rodada**: assimetria forte entre times que precisam ganhar vs times com vaga garantida.
...
|---|---|---|---|---|
| 41 | Noruega | 1 | 0 | Senegal |
| 42 | França | 2 | 0 | Iraque |
| 43 | Argentina | 2 | 0 | Áustria |
| 44 | Jordânia | 0 | 2 | Argélia |
| 45 | Inglaterra | 2 | 0 | Gana |
| 46 | Panamá | 0 | 2 | Croácia |
| 47 | Portugal | 1 | 0 | Uzbequistão |
| 48 | Colômbia | 2 | 0 | Congo (RD) |
| 49 | Escócia | 0 | 1 | Brasil |
| 50 | Marrocos | 2 | 0 | Haiti |
| 51 | Suíça | 1 | 1 | Canadá |
| 52 | Bósnia-Herzegovina | 2 | 0 | Qatar |
| 53 | República Tcheca | 0 | 2 | México |
| 54 | África do Sul | 1 | 2 | Coreia do Sul |
| 55 | Curaçao | 0 | 3 | Costa do Marfim |
| 56 | Equador | 1 | 1 | Alemanha |
| 57 | Japão | 0 | 1 | Suécia |
| 58 | Tunísia | 1 | 2 | Países Baixos |
| 59 | Turquia | 1 | 0 | Estados Unidos |
| 60 | Paraguai | 0 | 1 | Austrália |
| 61 | Noruega | 0 | 1 | França |
| 62 | Senegal | 2 | 0 | Iraque |
| 63 | Egito | 2 | 1 | Irã |
| 64 | Nova Zelândia | 0 | 5 | Bélgica |
| 65 | Cabo Verde | 0 | 1 | Arábia Saudita |
| 66 | Uruguai | 0 | 0 | Espanha |
| 67 | Panamá | 0 | 3 | Inglaterra |
| 68 | Croácia | 2 | 0 | Gana |
| 69 | Argélia | 1 | 1 | Áustria |
| 70 | Jordânia | 0 | 3 | Argentina |
| 71 | Colômbia | 1 | 0 | Portugal |
| 72 | Congo (RD) | 0 | 2 | Uzbequistão |
```
...
```
<!-- ia: <nome> <versão>; data: 2026-06-22; versao: v2; modo: <Deep Research|Extended Thinking|DeepSearch|...> -->
```

Exemplos válidos:
- `<!-- ia: ChatGPT 5 Pro; data: 2026-06-22; versao: v2; modo: Deep Research -->`
- `<!-- ia: Claude Opus 4.8; data: 2026-06-22; versao: v2; modo: Extended Thinking + Web Search -->`
- `<!-- ia: Gemini 2.5 Pro; data: 2026-06-22; versao: v2; modo: Deep Research -->`
- `<!-- ia: Grok 4 Heavy; data: 2026-06-22; versao: v2; modo: DeepSearch + Think -->`
...
2. As **32 linhas** estão presentes e na mesma ordem (jogos 41–72)
3. Todas as células de Gols A e Gols B estão preenchidas com inteiros ≥ 0
4. Nada além do comentário e da tabela na resposta
5. Usei os resultados dos jogos 1–40 da seção 1 como fatos fixos — não busquei nem questionei esses resultados
6. Pesquisei informação atualizada sobre estado das seleções, odds e cenários de classificação
7. Analisei especificamente os cenários de última rodada (jogos 53–72) antes de palpitá-los
