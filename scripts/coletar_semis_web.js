#!/usr/bin/env node
/*
 * Coleta Semis via interface WEB, dirigindo o Chrome do usuário via CDP.
 *
...
const PROMPT_PATH = path.join(ROOT, "config", "prompts", "ia-palpiteira-semis-web.md");
const RESULTADOS_PATH = path.join(ROOT, "data", "resultados", "jogos.md");
const OUT_DIR = path.join(ROOT, "data", "palpites_semis");

// 2 jogos das Semis (101-102)
const MM_JOGOS = [101, 102];
const LIMIAR = 2;
...
function salvar(slug, modo, conteudo) {
  ...
  const head = [
    ...
    "",
    `# Palpite Semis — ${slug} (interface web)`,
    "",
    "",
  ].join("\n");
...
async function extrairResposta(page, cfg) {
  return await page.evaluate(({ sel, jogos, limiar }) => {
    ...
    // 1) tabela dentro da última mensagem do assistente — limiar: 2 jogos
    if (el) {
    ...
    function reconstruirLinhas(text) {
      ...
      for (let i = 0; i < ls.length; i++) {
        if (jset.has(ls[i]) && /^(Semis|Quartas|Oitavas|R32)\b/i.test(ls[i + 1] || "")) {
