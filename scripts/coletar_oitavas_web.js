#!/usr/bin/env node
/*
 * Coleta Oitavas via interface WEB, dirigindo o Chrome do usuário via CDP.
 *
...
const PROMPT_PATH = path.join(ROOT, "config", "prompts", "ia-palpiteira-oitavas-web.md");
const RESULTADOS_PATH = path.join(ROOT, "data", "resultados", "jogos.md");
const OUT_DIR = path.join(ROOT, "data", "palpites_oitavas");

// 8 jogos das Oitavas (89-96)
const MM_JOGOS = [89, 90, 91, 92, 93, 94, 95, 96];
const LIMIAR = 8;
...
function salvar(slug, modo, conteudo) {
  ...
  const head = [
    ...
    "",
    `# Palpite Oitavas — ${slug} (interface web)`,
    "",
    "",
  ].join("\n");
...
async function extrairResposta(page, cfg) {
  return await page.evaluate(({ sel, jogos, limiar }) => {
    ...
    // 1) tabela dentro da última mensagem do assistente — limiar: 8 jogos
    if (el) {
    ...
    // Âncora: número do jogo seguido de uma linha "Oitavas" -> 9 células por jogo.
    function reconstruirLinhas(text) {
      const ls = (text || "").split("\n").map((s) => s.replace(/\s+/g, " ").trim()).filter(Boolean);
      const jset = new Set(jogos.map(String));
      const porJogo = {};
      for (let i = 0; i < ls.length; i++) {
        if (jset.has(ls[i]) && /^(Oitavas|R32)\b/i.test(ls[i + 1] || "")) {
