#!/usr/bin/env node
/*
 * Coleta v3 via interface WEB, dirigindo o Chrome do usuário via CDP.
 *
 ...
 * senão pelo host, senão abre a URL da conversa), cola o prompt v3-web, envia,
 * espera a resposta terminar, extrai o texto e salva em
 * data/palpites_v3/<slug>-web.md. Não commita nada.
 *
 * Uso:
 *   node scripts/coletar_v3_web.js --list                 # lista abas abertas (diagnóstico)
 *   node scripts/coletar_v3_web.js --site grok            # roda 1 site
 *   node scripts/coletar_v3_web.js --site grok --dry      # cola mas NÃO envia
 *   node scripts/coletar_v3_web.js --all                  # roda todos, um a um
 *   node scripts/coletar_v3_web.js --cdp http://localhost:9222
 */
...
const PROMPT_PATH = path.join(ROOT, "config", "prompts", "ia-palpiteira-v3-web.md");
const PALP_V2_DIR = path.join(ROOT, "data", "palpites_v2");
const OUT_DIR = path.join(ROOT, "data", "palpites_v3");
const V3_JOGOS = [61, 62, 67, 68, 69, 70, 71, 72];

// ---------------------------------------------------------------------------
// Config por site. url = link da conversa específica (recupera contexto v1/v2).
...
// Seletores são "melhor esforço" do DOM atual — vamos iterar site a site.
// ---------------------------------------------------------------------------
const SITES = {
  chatgpt: {
  ...
  gemini: {
    ...
    // /u/1/ = 2ª conta Google logada no perfil (giordanorec). A conversa v2/v3
    // está nessa conta; sem o /u/1/ cai na conta UFPE (/u/0) e abre chat novo.
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (t === "--dry") a.dry = true; // com --run: cola mas NÃO envia
    else if (t === "--nowait") a.nowait = true; // com --run: envia mas NÃO espera
...
function lerPalpitesV2(slug) {
  const cands = [slug, slug.toLowerCase()];
  for (const c of cands) {
    const p = path.join(PALP_V2_DIR, `${c}.md`);
    if (fs.existsSync(p)) return fs.readFileSync(p, "utf8");
  }
  return null;
}

function tabelaV2(textoMd) {
  const linhas = [
    "| Jogo | Time A | Gols A (seu v2) | Gols B (seu v2) | Time B |",
    "|---|---|---|---|---|",
  ];
  if (!textoMd) return "(você não registrou palpites v2 para estes 8 jogos)";
  const porJogo = {};
  for (const ln of textoMd.split("\n")) {
    const m = ln.match(/^\|\s*(\d+)\s*\|.*\|\s*([^|]+?)\s*\|\s*(\d+)\s*\|\s*(\d+)\s*\|\s*([^|]+?)\s*\|\s*$/);
    if (m) {
      const num = parseInt(m[1], 10);
      porJogo[num] = { timeA: m[2].trim(), ga: m[3], gb: m[4], timeB: m[5].trim() };
    }
  }
  let teve = false;
  for (const n of V3_JOGOS) {
    const r = porJogo[n];
    if (!r) continue;
    teve = true;
    linhas.push(`| ${n} | ${r.timeA} | ${r.ga} | ${r.gb} | ${r.timeB} |`);
  }
  return teve ? linhas.join("\n") : "(você não registrou palpites v2 para estes 8 jogos)";
}

function montarPrompt(slug) {
  let base = fs.readFileSync(PROMPT_PATH, "utf8");
  const idx = base.indexOf("# Briefing v3");
  if (idx > 0) base = base.slice(idx);
  const v2 = tabelaV2(lerPalpitesV2(slug));
  return base.replace("{{PALPITES_V2}}", v2);
}
...
async function extrairResposta(page, cfg) {
  return await page.evaluate(({ sel, jogos }) => {
    ...
    // escolhe a melhor <table> dentro de um escopo (mais jogos v3, depois mais linhas)
    ...
    if (el) {
      ...
      if (b && b.hits >= 8) return { md: b.md, via: "table", hits: b.hits, rows: b.rows };
    }
    // 2) fallback: melhor tabela do documento inteiro (vários sites não casam no seletor)
    ...
    // Âncora: número do jogo seguido de uma linha "Grupo X" -> 9 células por jogo.
    function reconstruirLinhas(text) {
      const ls = (text || "").split("\n").map((s) => s.replace(/\s+/g, " ").trim()).filter(Boolean);
      const jset = new Set(jogos.map(String));
      const porJogo = {};
      for (let i = 0; i < ls.length; i++) {
        if (jset.has(ls[i]) && /^Grupo\b/i.test(ls[i + 1] || "")) {
          const cells = ls.slice(i, i + 9);
          if (cells.length === 9) porJogo[ls[i]] = "| " + cells.join(" | ") + " |"; // última ocorrência
        }
      }
    ...
    // 2c) markdown em texto com pipes, possivelmente quebrado em várias linhas
    // (Manus): junta tudo numa string e extrai 9 células por jogo via regex.
    ...
    // 3) tabela parcial dentro da mensagem (hits<8 mas existe)
    if (el) {
      const b = melhorTabela(el);
  ...
  }, { sel: cfg.assistant, jogos: V3_JOGOS });
}

async function esperarFim(page, cfg, { maxMs = 900000, estavelMs = 6000 } = {}) {
  // Pronto quando: (a) há tabela/código com os 8 jogos e ficou estável; ou
  ...
  while (Date.now() - t0 < maxMs) {
    ...
    const estruturado = (r.via === "table" || r.via === "code" || r.via === "linhas" || r.via === "md") && r.hits >= 8;
    if (estruturado && estavel) return r.md;
    if (!gerando && estavel && r.hits >= 8) return r.md;
  }
...
async function inspecionar(browser, key) {
  ...
  // quais seletores de input existem/visíveis?
  ...
  // botão de stop existe agora? (não deveria, fora de geração)
  if (cfg.stop) {
...
// Só extrai + salva a resposta atual (sem recolar/reenviar). Útil quando uma
// geração já está em curso ou terminou e queremos recuperar o resultado.
async function coletarSite(browser, key) {
  ...
  if (r.hits < 8) {
    console.log("  !! menos de 8 jogos — pode ainda estar gerando. esperando...");
...
async function rodarSite(browser, key, args) {
  ...
  const prompt = montarPrompt(cfg.slug);
