#!/usr/bin/env node
/*
 * Coleta Quartas via interface WEB, dirigindo o Chrome do usuário via CDP.
 *
...
const PROMPT_PATH = path.join(ROOT, "config", "prompts", "ia-palpiteira-quartas-web.md");
const RESULTADOS_PATH = path.join(ROOT, "data", "resultados", "jogos.md");
const OUT_DIR = path.join(ROOT, "data", "palpites_quartas");

// 4 jogos das Quartas (97-100)
const MM_JOGOS = [97, 98, 99, 100];
const LIMIAR = 4;
...
function salvar(slug, modo, conteudo) {
  ...
  const head = [
    ...
    "",
    `# Palpite Quartas — ${slug} (interface web)`,
    "",
    "",
  ].join("\n");
...
async function extrairResposta(page, cfg) {
  return await page.evaluate(({ sel, jogos, limiar }) => {
    ...
    // 1) tabela dentro da última mensagem do assistente — limiar: 4 jogos
    if (el) {
    ...
    function reconstruirLinhas(text) {
      ...
      for (let i = 0; i < ls.length; i++) {
        if (jset.has(ls[i]) && /^(Quartas|Oitavas|R32)\b/i.test(ls[i + 1] || "")) {
...
async function esperarFim(page, cfg, { maxMs = 900000, estavelMs = 6000 } = {}) {
  // Pronto quando: (a) há tabela/código com os 4 jogos e ficou estável; ou
...
async function coletarSite(browser, key, args) {
  ...
  if (r.hits < LIMIAR) {
    console.log("  !! menos de 4 jogos — pode ainda estar gerando. esperando...");
