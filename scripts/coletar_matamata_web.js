#!/usr/bin/env node
/*
 * Coleta mata-mata R32 via interface WEB, dirigindo o Chrome do usuário via CDP.
 *
 ...
 * senão pelo host, senão abre a URL da conversa), cola o prompt mata-mata-web,
 * envia, espera a resposta terminar, extrai o texto e salva em
 * data/palpites_matamata/<slug>-web.md. Não commita nada.
 *
 * Uso:
 *   node scripts/coletar_matamata_web.js --list                 # lista abas abertas (diagnóstico)
 *   node scripts/coletar_matamata_web.js --site grok            # roda 1 site
 *   node scripts/coletar_matamata_web.js --site grok --dry      # cola mas NÃO envia
 *   node scripts/coletar_matamata_web.js --all                  # roda todos, um a um
 *   node scripts/coletar_matamata_web.js --cdp http://localhost:9222
 */
...
const PROMPT_PATH = path.join(ROOT, "config", "prompts", "ia-palpiteira-mata-mata-web.md");
const RESULTADOS_PATH = path.join(ROOT, "data", "resultados", "jogos.md");
const OUT_DIR = path.join(ROOT, "data", "palpites_matamata");
...
function salvar(slug, modo, conteudo) {
  ...
  const head = [
    ...
    "",
    `# Palpite mata-mata R32 — ${slug} (interface web)`,
    "",
    "",
  ].join("\n");
...
// Limiar principal: hits >= 16 (todos os jogos R32).
async function extrairResposta(page, cfg) {
  return await page.evaluate(({ sel, jogos }) => {
    ...
    // escolhe a melhor <table> dentro de um escopo (mais jogos R32, depois mais linhas)
    ...
    // 1) tabela dentro da última mensagem do assistente — limiar: 16 jogos
    if (el) {
      const b = melhorTabela(el);
      if (b && b.hits >= 16) return { md: b.md, via: "table", hits: b.hits, rows: b.rows };
    }
    ...
    // Âncora: número do jogo seguido de uma linha "R32" -> 9 células por jogo.
    function reconstruirLinhas(text) {
      const ls = (text || "").split("\n").map((s) => s.replace(/\s+/g, " ").trim()).filter(Boolean);
      const jset = new Set(jogos.map(String));
      const porJogo = {};
      for (let i = 0; i < ls.length; i++) {
        if (jset.has(ls[i]) && /^R32\b/i.test(ls[i + 1] || "")) {
    ...
    // 3) tabela parcial dentro da mensagem (hits<16 mas existe)
    if (el) {
      const b = melhorTabela(el);
  ...
  }, { sel: cfg.assistant, jogos: MM_JOGOS });
}

async function esperarFim(page, cfg, { maxMs = 900000, estavelMs = 6000 } = {}) {
  // Pronto quando: (a) há tabela/código com os 16 jogos e ficou estável; ou
  ...
  while (Date.now() - t0 < maxMs) {
    ...
    const estruturado = (r.via === "table" || r.via === "code" || r.via === "linhas" || r.via === "md") && r.hits >= 16;
    if (estruturado && estavel) return r.md;
    if (!gerando && estavel && r.hits >= 16) return r.md;
  }
...
async function coletarSite(browser, key, args) {
  ...
  if (r.hits < 16) {
    console.log("  !! menos de 16 jogos — pode ainda estar gerando. esperando...");
